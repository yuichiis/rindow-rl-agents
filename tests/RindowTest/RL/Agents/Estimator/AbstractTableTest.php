<?php
namespace RindowTest\RL\Agents\Estimator\AbstractTableTest;
...
class TestTable extends AbstractTable
{
    ...
    public function initialize() : void // : Operation
    {
        $la = $this->la;
        // initialize Q table
        $this->table = $la->alloc([$this->numStates,$this->numActions],dtype:NDArray::float32);
        $la->zeros($this->table);
    }
}
...
//
    ...
    public function testInitTable()
    {
        ...
        $trues = $la->zeros($la->alloc([$numStates,$numActions],dtype:NDArray::float32));
        $network = new TestTable($la,$numStates,$numActions);
        $table = $network->table();
        //echo "table=".$mo->toString($table,indent:true)."\n";
        //echo "trues=".$mo->toString($trues,indent:true)."\n";
        //$this->assertEquals($trues->toArray(),$table->toArray());
        $this->assertEquals($trues->toArray(),$table->toArray());

        $this->assertEquals([1],$network->stateShape());
        $this->assertEquals(3,$network->numActions());
        $this->assertTrue(true);
    }
...
//
    ...
    //    $network = new TestTable($la,$rules);
//
    //    $qValues = $network->getActionValues($la->array([[0],[1],[2]],dtype:NDArray::int32));
...
//
    ...
    public function testGetQValuesNormal()
    {
        $mo = $this->newMatrixOperator();
        $la = $mo->la();
        ...
        $states = $la->array([[0],[2],[1]],dtype:NDArray::int32);
        $qValues = $network->getActionValues($states);
        //$qValues = $la->axpy($la->copy($qValues),$la->copy($qValues),-1);
        //$qValues = $la->nan2num($qValues,1);
        //echo $mo->toString($qValues,null,true);
        $this->assertEquals([
