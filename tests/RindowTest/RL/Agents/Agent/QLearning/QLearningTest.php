...
class QLearningTest extends TestCase
{
    ...
    //    $agent = new QLearning($la,$rules,$policy,eta:0.1,gamma:0.9,mo:$mo);
    //    foreach($fixedActions as $ofs => $actions) {
    //        for($i=0;$i<100;$i++) {
    //            $state = $la->array([$ofs],dtype:NDArray::int32);
    ...
    public function testAction()
    {
        ...
        $policy = new AnnealingEpsGreedy($la,start:1.0,stop:0.01,decayRate:0.01);
        $agent = new QLearning($la,$numStates,$numActions,$policy,eta:0.1,gamma:0.9,mo:$mo);
        foreach($fixedActions as $ofs => $actions) {
            for($i=0;$i<100;$i++) {
                $state = $la->array([$ofs],dtype:NDArray::int32);
                $info = ['validActions'=>$rules[$ofs]];
